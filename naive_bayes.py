...
def train_mle_estimator(train_images, train_labels):
    ...
    pi_mle = np.zeros((10, 1))
    for j in range(10):
        pi_mle[j] = np.sum(train_labels.T[j])/train_images.shape[0]
    theta_mle = np.zeros((784, 10))
    label_t = train_labels.T
    image_t = train_images.T
    for k in range(0, 784):
        for c in range(10):
            theta_mle[k][c] = (image_t[k].dot(label_t[c]) / np.sum(label_t[c]))
...
def train_map_estimator(train_images, train_labels):
    ...
    pi_map = np.zeros((10,))
    for j in range(10):
        pi_map[j] = np.sum(train_labels.T[j]) / train_images.shape[0]
    theta_map = np.zeros((784, 10))
    label_t = train_labels.T
    image_t = train_images.T
    for k in range(0, 784):
        for c in range(10):
            theta_map[k, c] = (2 + (label_t[c].dot(image_t[k]))) / (4 + pi_map[c] * train_labels.shape[0])
...
def log_likelihood(images, theta, pi):
    ...
    log_pi_t = log_pi.T
    theta_t = theta.T
    one_theta = np.ones((784, 10)) - theta
    one_theta_t = one_theta.T
    for i in range(images.shape[0]):
        xj = images[i]
        one_xj = np.ones((1, 784)) - xj
        plus_term = xj.dot(np.log(theta)) + one_xj.dot(np.log(one_theta))
        summation = 0
        for j in range(10):
            cur_term = pi[j] * np.exp(xj.dot(np.log(theta_t[j])) + one_xj.dot(np.log(one_theta_t[j])))
            summation += cur_term[0]
        minus_term = np.log(summation)
        log_like[i] = log_pi_t + plus_term - minus_term
    return log_like
